...
| 周次 | 日期 | XJ | PF | LN | 备注 |
|------|------|----|----|-----|------|
| Week 1 | 02/15/2026 | **3h** | 10h | 15–20h | XJ 本周 3h |
| Week 2 | 02/22/2026 | 1.5h | 3h | 4.5–6h | **仅 30% 工作量** |
| Week 3 | 03/01/2026 | 5h | 10h | **10h** | LN 本周 10h；XJ 可开始美国数据库 |
| Week 4 | 03/08/2026 | 5h | 10h | 15–20h | 正常 |
| Week 5 | 03/15/2026 | 5h | 10h | 15–20h | 正常 |
...
**后端/前端工作量平衡（PF vs LN）：** PF 每周 10h 且承担全部前端，若同时做大量后端 API 容易过载。因此将部分**非 AI 后端 API** 分配给 LN，使 PF 在「前端+后端同周」时更偏前端、LN 多承担后端，保证总量可完成且分配合理：
- **Week 1**：扩展 `GET /students` 由 **LN** 负责（PF 专注 api-client 与前端）。
- **Week 3**：学生/标签相关 API（`GET/PUT /counselor/students/{id}`、标签 CRUD）由 **LN** 负责，PF 本周**仅做前端**（列表接 API、详情页、标签 UI），避免 PF 在 10h 内同时做 3 个 API + 3 个前端任务。
- LN 周工时多于 PF（15–20h vs 10h），且 Week 3 为 10h，上述后端任务与 Summary 一起在 Week 3 由 LN 完成在可接受范围内；若 LN 本周吃紧可优先保证 Summary + 学生 GET/PUT，标签 CRUD 可与 PF 协商谁做。
...
#### PF（10h）— 本周以前端为主，减轻后端负担

| # | 任务（类型） | 对应功能 | 交付物 |
|---|----------------|----------|--------|
| 1 | [Frontend] 统一 API 客户端：fetch 封装、Authorization token、错误处理、loading 状态 | F2–F5 共用 | `api-client` 模块（如 `src/lib/api-client.ts`） |
| 2 | [Frontend] 学生列表页接入新 API 的占位或试跑（可选，为 Week 3 做准备） | F2 | — |

#### LN（15–20h）— 承担后端架构 + GET /students 扩展 + Summary 骨架
...
| 3 | [API] `POST /counselor/students/{student_id}/summary` 接口占位（路由、权限校验、请求体校验） | F4 | Summary 接口骨架（可返回占位文本） |
| 4 | [AI] Summary 用 prompt 模板草稿：支持 focus_areas、custom_prompt 占位 | F4 | `prompt.py` 中 summary 模板草稿 |

**本周交付物汇总**：核心表设计文档、迁移步骤大纲；前端 api-client（PF）；扩展版 GET /students、后端重构、Summary 接口骨架与 prompt 草稿（LN）。

**里程碑**：前端可调通后端；数据库核心表设计就绪（Week 2 补全后即可执行迁移）；Summary 接口可联调。

---

### Week 2（02/22/2026）— 新年仅 30% 工作量
...
#### PF（10h）— 本周仅前端，后端学生/标签 API 由 LN 承担以平衡工作量

| # | 任务（类型） | 对应功能 | 交付物 |
|---|----------------|----------|--------|
| 1 | [Frontend] `/counselor/students` 接真实 API（替换 mock）；loading、错误、刷新 | F2 | 学生列表页接后端 |
| 2 | [Frontend] 学生详情页或模态框：信息展示 + 编辑表单 + 保存 | F3 | 学生详情/编辑 UI |
| 3 | [Frontend] 标签展示与添加/删除 UI（学生详情内或列表卡片） | F3 | 标签组件与交互 |

#### LN（10h）— 学生/标签 API + Summary API，与 PF 前端联调
...
| 4 | [AI] `POST /counselor/students/{student_id}/summary` 完整逻辑：组合 prompt、调用现有 AI、返回 summary | F4 | Summary 生成 API 可用 |
| 5 | [API] Summary 权限与参数校验；可选：summary 缓存 | F4 | 接口符合文档、缓存（若实现） |

**本周交付物汇总**：美国云 PostgreSQL + 迁移脚本与验证（XJ）；学生 GET/PUT、标签 CRUD、Summary 生成 API（LN）；学生列表/详情/标签前端（PF）。
...
### PF（前端为主；部分后端 API 已调至 LN 以平衡）

| 周 | 任务（类型 · 功能） | 交付物 |
|----|----------------------|--------|
| 02/15 | [Frontend] API 客户端（F2–F5 共用） | api-client |
| 02/22 | [Frontend] 注册：角色选择、密码强度（F1） | 注册表单增强 |
| 03/01 | [Frontend] 学生列表接 API、详情/编辑、标签 UI（F2/F3） | 学生列表/详情/标签前端 |
...
### LN（后端 AI + 更多后端 API，分担 PF 后端压力）

| 周 | 任务（类型 · 功能） | 交付物 |
|----|----------------------|--------|
| 02/15 | [API] 后端重构、扩展 GET /students；[API] Summary 接口占位；[AI] Summary prompt 草稿（F2/F4） | 后端重构、/students 扩展、Summary 骨架与 prompt 草稿 |
| 02/22 | [AI] Summary prompt 定稿；[API] 可选 GET summary（F4） | Prompt 定稿、GET summary（可选） |
| 03/01 | [API] 学生 GET/PUT、标签 CRUD；[AI] POST summary 完整实现（F2/F3/F4） | 学生与标签 API、Summary 生成 API |
...
- **LN 03/01 仅 10h**：该周 LN 负责学生/标签 API + Summary API，若 10h 吃紧，优先保证 Summary 生成 + 学生 GET/PUT；标签 CRUD 可与 PF 协商由谁实现或延后半周。PF 本周仅前端，不增加后端负担。
